...
int main(int argc, char** argv) {
	...
	// --- preprocesss 2 molecules with prePCA.py--------------------------
	if (c.doPrePCA) {
		string mol1name(argv[indexArray[1]]);
		string ls = GetStdoutFromCommand("python prePCA.py " + mol1name);
		if (ls[0] != 'S' || ls[1] != ':') {
			sprintf(msg, "Molecular prePCA error in %s", argv[indexArray[1]]);
			message(msg, MSG_ERROR);
			return -1;
		}
		string mol2name(argv[indexArray[2]]);
		ls = GetStdoutFromCommand("python prePCA.py " + mol2name);
		if (ls[0] != 'S' || ls[1] != ':') {
			sprintf(msg, "Molecular prePCA error in %s", argv[indexArray[2]]);
			message(msg, MSG_ERROR);
			return -1;
		}
	}
	...
	for (unsigned int imol = 0; imol < db.size(); imol++) {
		...
		ini->getMolVariable()->setTanimoto(ini->getMolVariable()->getAtomsXYZ(),
				...
				ini->getSameVanDerWaalsRadius());
		//}
		//cout << "Tanimoto query: "<<ini->getMolQuery()->tanimoto<<endl;
		//cout << "Tanimoto target: "<<ini->getMolVariable()->tanimoto<<endl;
		...
		if ((minMol / maxMol) < ini->getLowLimit()){
			cerr << "optipharm: -- Maximum posible score is lower than limit used: "<< ini->getLowLimit()<<"\n";
			continue;
		}
