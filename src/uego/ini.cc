...
// returns true if success, false if not
	...
	char FailFlag, IsDouble = d != NULL, *pos, msg[100], white[15] =
			" \n\v\b\r\f\t";
	long i;
	double db;
...
void Ini::updateVectorIni() {
	...
	for (long i = 0; i < this->Dimension(); ++i) {
		sqrsum += (this->Lowb(i) - this->Upb(i))
				* (this->Lowb(i) - this->Upb(i));
	};
	...
	for (int i = 1; i < this->Levels(); ++i){
		//cout << "i: "<<i << "\n";
		evals[i] = (unsigned long) (this->MaxSpecNumber() * r[0] * threshold
				/ V(r[i]));
		//cout << "Evals: "<< evals[i] << " MaxSpecNumber "<< this->MaxSpecNumber() << ", r[0]: " << r[0] << ", V(r[i]): " <<V(r[i]) << "\n";
	}
