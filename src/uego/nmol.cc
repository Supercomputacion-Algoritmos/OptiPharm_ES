...
SearchSpElement* NDimMolElement::RandNew2(SearchSpElement* e) {
// uniform distribution in search space
...
SearchSpElement* NDimMolElement::RandNew3(SearchSpElement* e) {
// uniform distribution in search space
...
SearchSpElement* NDimMolElement::RandNew2(short radind, SearchSpElement* e) {
// uniform distribution in given area (a hyper-rectangle)
	...
	if (radind == 0)
		return RandNew2(e); // default for root
...
// --- new empty element
	result = new NDimMolElement(dim);
	if (result == NULL || result->Fail()) {
		message("No memory in NDimMolElement::RandNew2(2)", MSG_ERROR);
		return (SearchSpElement*) NULL;
	};
...
SearchSpElement* NDimMolElement::RandNew3(short radind, SearchSpElement* e) {
// uniform distribution in given area (a hyper-rectangle)
	...
	if (radind == 0)
		return RandNew3(e); // default for root
...
// --- new empty element
	...
	if (number < 4) {
		...
		//only change the translation x[7-9]
		for (long i = 7; i < dim; ++i) {
			result->x[i] = UegoDoubleRand(r->x[i] - rad, r->x[i] + rad);
			if (result->x[i] < 0.0)
				result->x[i] = 0.0;
			else if (result->x[i] > 1.0)
...
SearchSpElement* NDimMolElement::BetweenNew2(SearchSpElement* e1,
		SearchSpElement* e2) {
...
SearchSpElement* NDimMolElement::BetweenNew3(SearchSpElement* e1,
		SearchSpElement* e2) {
